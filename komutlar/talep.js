...
exports.run = (client, message, args) => {
 message.delete();
    message.guild.createChannel(`talep-${message.author.username}`, 'text').then(ch => {
        ...
        .setAuthor("» Presidente | Destek Sistemi")
        .setDescription("**Yetkili Arkadaşlar Sizinle İlgilenecektir.\nDestek Talebini İptal Etmek İçin \`[p!kapat]\` Yazabilirsin!**")
        .setFooter(' Presidente | Destek Sistemi', client.user.avatarURL)
        .setTimestamp()
        ch.send(embed).catch()
        ch.send("@everyone")
        ch.send("@here")
        ch.awaitMessages((msg)=> {
            if (msg.content === "p!kapat") {
